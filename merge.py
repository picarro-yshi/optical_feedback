...
if __name__ == "__main__":
    ...
    # def pool_handler():
    #     p = Pool()
    #     # p.map(work_log, optical_file_list)
    #     p.map(work_log, [(d, matchDict) for d in optical_file_list])

    # pool_handler()

    # p = Pool(processes=4)
    # p.starmap(work_log, [(d, matchDict) for d in optical_file_list])
